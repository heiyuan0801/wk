...
不会保存提示词或模型输出，但会保存最多 1,024 字符的错误详情。`GET /requests?limit=50`
...
仪表盘可选择最近 24 小时、7 天、30 天、90 天或全部时间，展示范围内的总请求数、总 Token、非缓存输入、输出、缓存读取和缓存命中率。`GET /stats?range=24h` 提供同一统计，`range` 也可取 `7d`、`30d`、`90d`、`all`；有限范围基于仍在保留期内的请求日志聚合，全部时间使用累计指标。
